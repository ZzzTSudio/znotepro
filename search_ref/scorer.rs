...
pub fn score_documents(index: &SearchIndex, query: &ParsedQuery) -> Vec<ScoredDoc> {
    ...
    // AND filter on must_terms
    if !query.must_terms.is_empty() {
        let must_doc_sets: Vec<Vec<u32>> = query.must_terms.iter()
            .map(|term| {
        ...
        if let Some(first) = must_doc_sets.first() {
            let intersection: Vec<u32> = first.iter()
    ...
    // Phrase filter: doc must contain all specified phrases
    if !query.must_phrases.is_empty() {
        let phrases = &query.must_phrases;
        doc_scores.retain(|&doc_id, (_, boosts)| {
            let all_match = phrases.iter().all(|tokens| {
                tokens.len() < 2 || check_phrase_match(index, doc_id, tokens)
            });
            if all_match && !boosts.contains(&"phrase_match".to_string()) {
    ...
    if !query.exclude_terms.is_empty() {
        let exclude_docs: Vec<u32> = query.exclude_terms.iter()
            .flat_map(|term| {
                index.postings.get(term)
    ...
    for (&doc_id, (score, boosts)) in doc_scores.iter_mut() {
            ...
            if let Some(doc) = index.documents.get(&doc_id) {
                ...
                if title_lower.contains(&no_space) || path_lower.contains(&no_space) {
                    *score *= EXACT_MATCH_BOOST;
                    if !boosts.contains(&"exact_match".to_string()) {
        ...
        // Phrase boost (already detected above)
        if boosts.contains(&"phrase_match".to_string()) {
            *score *= PHRASE_BOOST;
        }
    }

    let mut results: Vec<ScoredDoc> = doc_scores.into_iter()
        .map(|(doc_id, (score, boosts))| ScoredDoc { doc_id, score, boost_reasons: boosts })
        .collect();
...
fn check_phrase_match(index: &SearchIndex, doc_id: u32, phrase_tokens: &[String]) -> bool {
    ...
    let first_positions = match index.postings.get(first_term) {
        Some(postings) => {
            match postings.iter().find(|p| p.doc_id == doc_id) {
                Some(p) => &p.positions,
                None => return false,
            }
        }
    ...
    'outer: for &start_pos in first_positions {
        for (offset, term) in phrase_tokens.iter().enumerate().skip(1) {
            ...
            let found = index.postings.get(term)
